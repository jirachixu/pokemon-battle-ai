...
I've decided to scrape a bunch of teams from the VGCPastes spreadsheet, including only teams with EVs and that placed in tournaments or went to Regionals or Worlds, which ensures a baseline quality. So far, this gives 85 teams and I will update this if/when necessary in the future as more teams are added or regulation changes. I also have used these teams to get some baseline item probability distributions for each Pokemon in at least one team to use for item encoding when no item is seen on a Pokemon. I also added in the state encoding the effective multiplier on the moves of each Pokemon so the agent can have a better idea of how much damage moves actually do (STAB, weather, terrain, variable base power moves, etc.).

I started training (went to 300k turns), and the results are promising in that even after this long, the rollout mean reward is still following a consistent upwards trajectory, so I will keep going tomorrow. I think this is probably because the model now more accurately knows exactly how much damage moves do so the variation seen previously in clicking 2 moves with the same base power in conditions suited for only one move (e.g. flamethrower in sun vs. ice beam in sun, flamethrower clearly has higher effective base power) is now gone, which was probably a source of significant noise before.
